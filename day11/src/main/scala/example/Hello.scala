...
object Hello extends App {
  ...
  def parseData(input: String): Grid = {
    Grid(input.split("\n").toVector.map(_.toVector.map(x => Octopus(x.asDigit, false))))
  }
  println(play2(parseData(Resource.getAsString("input.txt"))))
}
...
final case class Grid(values: Vector[Vector[Octopus]]) { self =>
  ...
  def get(row: Int, col: Int): Option[Octopus] = {
    if (values.isDefinedAt(row) && values(row).isDefinedAt(col)) Some(values(row)(col)) else None}
  ...
  def touchOctopus(row: Int, col: Int): Grid = {
    get(row, col) match {
      ...
      case Some(Octopus(9, false)) =>
        neighbours(row, col).filterNot(_.octopus.flashed).foldLeft(updated(row, col, Octopus.freshlyFlashed)) { case (acc, oct) =>
          acc.touchOctopus(oct.row, oct.col)  
        }
      case Some(oct @ Octopus(n, false)) => updated(row, col, oct.copy(value = n + 1))
      case None => self
    }
  }
