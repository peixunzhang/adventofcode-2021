...
object Hello extends App {
  ...
  println(parseData(Resource.getAsString("input.txt")).threeLargestBasinsSize)
}

final case class Map(map: Vector[Vector[Int]]) {
  def get(row: Int, col: Int): Option[Int] =
    if (map.isDefinedAt(row) && map(row).isDefinedAt(col)) Some(map(row)(col)) else None
  ...
  def getLowPointOld(col: Int, row: Int): Option[Int] = 
    {
      ...
      if (up.zip(target).fold(true){case (u, t) => t < u} &&
      down.zip(target).fold(true){case (d, t) => t < d} &&
      right.zip(target).fold(true){case (r, t) => t < r} &&
      left.zip(target).fold(true){case (l, t) => t < l}) target else None
  }

  def getLowPoint(row: Int, col: Int): Option[LowPoint] = {
    get(row, col).flatMap { t => 
      if (getNeighbours(row, col).forall(x => x.value > t)) Some(LowPoint(Point(t, row, col))) else None
    }    
  }
  ...
  def getBasinSize(lowPoint: LowPoint): Int = {
    def go(neighbour: Point): Set[Point] = {
      Set(neighbour) ++ getNeighbours(neighbour.row, neighbour.col).filter(x => x.value > neighbour.value && x.value != 9).toSet.flatMap(go(_))
    }
    go(lowPoint.value).size
  }

  def threeLargestBasinsSize: Int = {
    val allBasins = map.zipWithIndex.flatMap { case (cs, r) => cs.indices.flatMap { c => getLowPoint(r, c).map(getBasinSize) } }
    allBasins.sortBy(-_).take(3).product
  }

  def riskSum: Int = map.zipWithIndex.foldLeft(0){case (n, (cs, r)) => cs.indices.foldLeft(n)((n, c) => getRisk(r, c)+n)}
}
